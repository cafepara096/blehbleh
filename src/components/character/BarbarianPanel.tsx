...
/** Preferencias de bárbaro guardadas en notes JSON liviano vía campo genérico del personaje.
 *  Usamos keys en notes no; mejor campos opcionales en character via any-safe:
 *  metamagicKnown-style arrays reutilizados donde aplica, y description en features.
 */
type BarbPrefs = {
  raging?: boolean;
...
function readPrefs(character: Character): BarbPrefs {
  try {
    const raw = (character as Character & { barbarianPrefs?: BarbPrefs }).barbarianPrefs;
    ...
    <div className="space-y-2">
      ...
      <div className="bg-red-50 border-2 border-red-400 rounded-xl p-3 text-sm">
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          ...
          <button
            type="button"
            onClick={() => setRageUses(rageCurrent - 1)}
        ...
        <p className="text-[10px] text-ink-600 mt-2">
          Recuperas 1 uso en descanso corto y todos en descanso largo. Sin armadura pesada.
          Resistencia a contundente/cortante/perforante; ventaja en Fue; no concentración ni
          conjuros.
        </p>
      </div>
        ...
        {knownMastery.length > 0 && (
          <ul className="mt-1 flex flex-wrap gap-1">
            {knownMastery.map((w) => (
              <li
                key={w.id}
                className="bg-white border border-amber-200 rounded px-1.5 py-0.5 text-[10px]"
              >
                {w.name}
              </li>
            ))}
        ...
        {showMastery && (
          <div className="mt-1 max-h-36 overflow-y-auto space-y-0.5 border-t border-amber-200 pt-1">
            ...
            <p className="text-[10px] text-ink-500 pt-1">
              Tras un descanso largo puedes cambiar una elección. Homebrew: edita la lista en
              datos / catálogo si añades armas propias.
            </p>
          </div>
        )}
      </div>
          ...
          {level >= 6 && (
            <div>
              <p className="text-[10px] font-semibold mb-0.5">
                Aspecto de lo salvaje (cambia en descanso largo)
              </p>
              <div className="flex flex-wrap gap-1">
      ...
      {isZealot && warriorDice?.uses && (
        <div className="bg-yellow-50 border border-yellow-400 rounded-lg p-2.5 text-xs">
          ...
          <p className="text-[10px] text-ink-600 mt-0.5">
            Acción adicional: gasta dados de la reserva y recuperas PG = total. Recarga en descanso
            largo.
          </p>
        </div>
      )}
      ...
      <p className="text-[10px] text-ink-500">
        Rasgos de clase y subclase se añaden/quitan al gestionar niveles. Puedes editar o crear
        sendas y rasgos como homebrew en la sección Clases.
      </p>
    </div>
  );
}
