...
export function CharacterWizard({ onComplete, onCancel }: Props) {
  ...
  const packData = classId
    ? (startingEquipment as Record<string, {
        fixed?: Omit<InventoryItem, 'id'>[];
        choices?: { id: string; label: string; options: { name: string; quantity?: number; damage?: string; damageType?: string; properties?: string[]; description?: string; equipped?: boolean }[] }[];
  ...
  const equipmentValid = (): boolean => {
    if (equipMode === 'gold' || !packData) return true;
    const choices = packData.choices || [];
    for (const choice of choices) {
      ...
      if (needsWeaponPick(opt.name || '')) {
        ...
        const got = (weaponPicks[choice.id] || []).filter(Boolean).length;
  ...
  const buildInventory = (): InventoryItem[] | undefined => {
    if (equipMode === 'gold' || !classId || !packData) return undefined;
    const items: InventoryItem[] = [];
    const pushItem = (raw: Record<string, unknown>) => {
      try {
        const expanded = expandStartingOption(raw as Parameters<typeof expandStartingOption>[0]);
        ...
        items.push({
          id: crypto.randomUUID(),
          name: String(raw.name || 'Objeto'),
          quantity: Number(raw.quantity) || 1,
        });
      }
    };
    for (const fixed of packData.fixed || []) pushItem(fixed as Record<string, unknown>);
    for (const choice of packData.choices || []) {
      ...
      if (/^(arma marcial|arma simple|cualquier arma)/i.test(opt.name || '')) continue;
      pushItem(opt as Record<string, unknown>);
    }
    return items;
  };
  ...
  const finish = () => {
    ...
    if (playerName.trim()) {
      character = { ...character, playerName: playerName.trim() };
    }
    ...
    if (equipMode === 'gold') {
      ...
      character = {
        ...character,
        inventory: [],
        currency: { ...(character.currency || { pp: 0, gp: 0, ep: 0, sp: 0, cp: 0 }), gp: (character.currency?.gp || 0) + gold },
      };
    }
            ...
            <div className="space-y-3">
              ...
              {selectedClass && subclassOptions.length > 0 && (
                <div className="border-t border-ink-200 pt-3">
                  <h3 className="font-bold text-sm mb-1">Subclase (opcional ahora)</h3>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {filterList(backgrounds).map((b) => (
                  ...
                  >
                    ...
                    {b.skillProficiencies?.length ? (
                      <p className="text-[10px] text-ink-500 mt-1">Hab.: {b.skillProficiencies.join(', ')}</p>
                    ) : null}
                  </button>
                ))}
              </div>
          ...
          {step === 'abilities' && (
            <div className="space-y-3">
              ...
              {abilityMethod === 'pointbuy' && (
                <p className="text-xs">
                  Puntos: <strong className={pbSpent === 27 ? 'text-green-700' : 'text-amber-700'}>{pbSpent}/27</strong>
                </p>
              )}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {ABILITIES.map((ab) => (
                  <div key={ab} className="bg-white border-2 border-ink-300 rounded-lg p-2">
                    ...
                    {abilityMethod === 'array' ? (
                      <select
                        className="w-full mt-1 border rounded px-1 py-1 text-sm font-bold"
                        value={arrayAssign[ab] ?? ''}
                      ...
                      >
                        ...
                        {STANDARD_ARRAY.map((v) => (
                          <option
                            key={v}
                            value={v}
                            disabled={Object.entries(arrayAssign).some(([k, val]) => k !== ab && val === v)}
                          >
                            {v}
                          </option>
                        ))}
                    ...
                    <div className="text-center text-xs text-ink-500 mt-0.5">
                      ...
                      {selectedRace && previewScores[ab] !== baseScores[ab] && (
                        <span className="text-green-700"> → {previewScores[ab]}</span>
                      )}
                    </div>
                    {abilityMethod === 'pointbuy' && (
              ...
              {skillOpts ? (
                <>
                  <p className="text-xs text-ink-600">
                    Elige <strong>{skillOpts.count}</strong> de la lista de {selectedClass?.name}. Competencia = bonificador de competencia.
                  </p>
                  <div className="grid sm:grid-cols-2 gap-1.5 max-h-56 overflow-y-auto">
                    {skillOpts.skills.map((sk) => {
                      const meta = SKILLS.find(
                        (s) => s.name.toLowerCase() === sk.toLowerCase() || s.id === sk.toLowerCase()
                      );
                      const selected = chosenSkills.includes(sk);
                      return (
                        <label
                          key={sk}
                          className={`flex gap-2 items-start p-2 rounded border cursor-pointer text-xs ${selected ? 'bg-crimson-50 border-crimson-400' : 'bg-white border-ink-200'}`}
                        >
          ...
          {step === 'equipment' && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setEquipMode('pack')}
                  className={`px-3 py-1.5 rounded-lg text-sm border-2 ${equipMode === 'pack' ? 'bg-crimson-700 text-white border-crimson-800' : 'bg-white border-ink-300'}`}
                >
                  Paquete
                </button>
                <button
                  type="button"
                  onClick={() => setEquipMode('gold')}
                  className={`px-3 py-1.5 rounded-lg text-sm border-2 ${equipMode === 'gold' ? 'bg-crimson-700 text-white border-crimson-800' : 'bg-white border-ink-300'}`}
                >
                  Oro
                </button>
              </div>
              {equipMode === 'gold' ? (
                <p className="text-sm text-ink-600">Empiezas con oro según clase (sin paquete de equipo).</p>
              ) : packData ? (
                <div className="space-y-3 text-sm">
                  {(packData.fixed || []).length > 0 && (
                    <div>
                      <h3 className="font-bold text-xs uppercase text-ink-500 mb-1">Equipo fijo</h3>
                      <ul className="space-y-0.5">
                        {(packData.fixed || []).map((it, i) => (
                          <li key={i} className="text-xs">
                            • {it.name}
                            {it.quantity && it.quantity > 1 ? ` ×${it.quantity}` : ''}
                            {packSummary(it.name) && (
                              <span className="block text-[10px] text-ink-500 ml-3">Incluye: {packSummary(it.name)}</span>
                            )}
                          </li>
                        ))}
                      </ul>
                  ...
                  {(packData.choices || []).map((choice) => (
                    <div key={choice.id} className="border border-ink-200 rounded-lg p-2 bg-white">
                      <p className="font-bold text-xs mb-1">{choice.label}</p>
                      <div className="space-y-1">
                        {choice.options.map((opt, ci) => (
                          <label key={ci} className="flex gap-2 items-start text-xs cursor-pointer">
                            <input
                              type="radio"
                              name={`eq-${choice.id}`}
                              checked={equipChoices[choice.id] === ci}
                              onChange={() => {
                                setEquipChoices((prev) => ({ ...prev, [choice.id]: ci }));
                                setWeaponPicks((prev) => {
                                  const next = { ...prev };
                            ...
                            <span>
                              ...
                              {packSummary(opt.name) && (
                                <span className="block text-[10px] text-ink-500">Incluye: {packSummary(opt.name)}</span>
                              )}
                            </span>
                          </label>
                        ))}
                        ...
                        return (
                          <div className="mt-2 p-2 bg-parchment-50 border border-ink-200 rounded-lg">
                            <p className="text-xs font-bold mb-1">
                              Elige {need === 2 ? '2 armas' : 'el arma'}
                              {optionIncludesShield(sel.name || '') ? ' (+ escudo)' : ''} ({selected.length}/{need})
                            </p>
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setWeaponPicks((prev) => {
                                        ...
                                        if (need === 1) return { ...prev, [choice.id]: [w.id] };
                                        const idxW = cur.indexOf(w.id);
                                        if (idxW >= 0) cur.splice(idxW, 1);
                                        else if (cur.length < need) cur.push(w.id);
                                        else cur = [...cur.slice(1), w.id];
                                        return { ...prev, [choice.id]: cur };
                                      });
                                    }}
                                    className={`text-left text-xs px-2 py-1.5 rounded border ${on ? 'border-crimson-600 bg-parchment-200' : 'border-ink-200 bg-white'}`}
                                  >
                                    <strong>{w.name}</strong>
                                    {count > 1 ? ` ×${count}` : ''}
                                    <span className="text-ink-500"> {w.damage} {w.damageType}</span>
                                  </button>
                                );
                              })}
                            </div>
          ...
          {step === 'review' && (
            <div className="space-y-3">
              ...
              {chosenSkills.length > 0 && (
                <p className="text-xs"><strong>Habilidades:</strong> {chosenSkills.join(', ')}</p>
              )}
            </div>
          )}
        </div>
